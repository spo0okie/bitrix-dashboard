	...
	*****************************************************************/
	...
	{	
		...
		while($arM = $rs->Fetch())
		{
			...
			{
				//вот тут мы обнаруживаем что последнее сообщение от ТП или юзера
				if((!CTicket::IsOwner($ticketID,intval($arM["OWNER_USER_ID"]))) && in_array(intval($arM["OWNER_USER_ID"]), $arSupportTeam))
				{
					$arFirstUserMessAfterSupportMess = null;
					$messAfterSupportMess = true;
				}

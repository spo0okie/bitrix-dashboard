...
function placeCanbanItemCorrect($li) {
    ...
    let $oldTd=$oldUl.parents('td.userColumn');
    /*if (!$oldUl.length) {
        console.log('No old UL');
    } else {
        if (!$oldTd.length) {
            console.log('No old TD');
        }
    }*/
    if ($ul && $ul.length) {
        $ul.append($li.detach());
        ...
        if ($oldTd.length) {
            //console.log('old period recalc');
            //console.log($oldTd);
            updateUserPeriodColumnTotals($oldTd);
        } else {
            //console.log('new item placed')
            //
        }
    } else {
        $li.remove();
